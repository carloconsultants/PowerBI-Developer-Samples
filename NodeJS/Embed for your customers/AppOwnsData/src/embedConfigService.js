...
async function getEmbedInfo() {
    ...
    try {
        ...
        } else if (config.paginatedReportId) {
            console.log("found Paginated Report Id");
            const embedParams = await getEmbedParamsForSinglePaginatedReport(config.workspaceId, config.paginatedReportId);
        }       
 ...
 * Get embed params for a single report for a single workspace
 * @param {string} workspaceId
 * @param {string} paginatedreportId
 * @param {string} additionalDatasetId - Optional Parameter
 * @return EmbedConfig object
 */
async function getEmbedParamsForSinglePaginatedReport(workspaceId, paginatedreportId, additionalDatasetId) {
    
    const reportInGroupApi = `https://api.powerbi.com/v1.0/myorg/groups/${workspaceId}/reports/${paginatedreportId}`;
    const headers = await getRequestHeader();
